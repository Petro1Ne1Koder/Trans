...
from func_savepostfix import SavePostfixCode
...
# завантаження ПОЛІЗ-програми у форматі .postfix 
    ...
    try:
      while self.numInstr < self.maxNumbInstr:
        ...
        if tok in ('int','float','l-val','r-val','label','bool'):
          ...
          self.numInstr = self.numInstr +1
          try:
            val = self.tableOfId[id][2]
            print(f'-------------- OUT: {id} = {val}')
          except:
            print(f'-------------- OUT: {id}')
        elif tok == 'in_op':
          
          inputval = input('Введіть значення\n').strip()
          if inputval in ('False','True'):
              lex,tok = self.postfixCode[self.numInstr - 1]
              typeOfvar = self.tableOfId[lex][1]
              if typeOfvar == 'bool':
                  self.stack.push((inputval,'bool'))
                  self.numInstr = self.numInstr +1
              else:
                raise TypeError
          elif isinstance(eval(inputval), int | float):
              lex,tok = self.postfixCode[self.numInstr - 1]
              typeOfvar = self.tableOfId[lex][1]
              if isinstance(eval(inputval), int) and typeOfvar == 'int':
                  self.stack.push((inputval,'int'))
                  self.numInstr = self.numInstr +1
              elif isinstance(eval(inputval), int | float) and typeOfvar == 'float':
                  self.stack.push((inputval,'float'))
                  self.numInstr = self.numInstr +1
              else:
                  raise TypeError
          else:
              raise ('SMTH NOT UNDERSTANDABLE')
        else: 
  ...
  def doIt(self,lex,tok):
    ...
    if (lex,tok) == ('=', 'assign_op'):
      ...
      if (tokL == 'bool' and tokR in ('int', 'float')) or (tokL == 'int' and tokR == 'float'): 
  ...
  def applyOperator(self,lexTypeValL,arthBoolOp,lexTypeValR):
    ...
    if (typeL == 'bool' and typeR in ('int', 'float')):
      raise PSMExcept(9)  # типи операндів відрізняються
                         # додати перевірка на інт не модже буть флоат а флоат може буть інт але перевод в флоат
    elif arthBoolOp == '+':
      value = valL + valR
    elif arthBoolOp == '-':
      value = valL - valR
...
pm1=PSM() 
pm1.loadPostfixFile("postfixcode")  #  завантаження .postfix - файла
